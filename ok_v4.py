...
def brique_check(ball_x, ball_y, brk_x, briques, ball_velocity_y, ball_velocity_x):
    ...
    if ball_y == 34 and ball_velocity_y < 0:
        for n in range(0,12,2):
            if (brk_x[n]<=ball_x<=brk_x[n+1] or brk_x[n]<=ball_x+1<=brk_x[n+1] or brk_x[n]<=ball_x+2<=brk_x[n+1]) and briques[n//2] != 0:
...
def update():
    ...
    global briques_x, briques_y, ball_x, ball_y, ball_velocity_x, ball_velocity_y, life, brk_x, briques, value


    ball_x, ball_y, ball_velocity_x, ball_velocity_y = ball_mvt(ball_x, ball_y, ball_velocity_x, ball_velocity_y)


    if ball_y == 34 or ball_y+3 == 30:
