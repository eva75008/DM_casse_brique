...
def brique_check(ball_x, ball_y, brk_x, briques, ball_velocity_y, ball_velocity_x, score):
    ...
    if 35 >= ball_y >= 33 and ball_velocity_y < 0:
        for n in range(0,12,2):
            if (brk_x[n]<=ball_x<=brk_x[n+1] or brk_x[n]<=ball_x+1<=brk_x[n+1] or brk_x[n]<=ball_x+2<=brk_x[n+1] or brk_x[n]<=ball_x+3<=brk_x[n+1]) and briques[n//2] != 0:
                ...
                return ball_x,  ball_y, ball_velocity_x, ball_velocity_y, score

    if 32 >= int(ball_y)+3 >= 29 and ball_velocity_y > 0:
        for n in range(0,12,2):
            if (brk_x[n]<=ball_x<=brk_x[n+1] or brk_x[n]<=ball_x+1<=brk_x[n+1] or brk_x[n]<=ball_x+2<=brk_x[n+1] or brk_x[n]<=ball_x+3<=brk_x[n+1]) and briques[n//2] != 0:
...
def timer(ball_velocity_x, ball_velocity_y):
    '''
    Cette fonction sert à augmenter la vitesse de la balle de au bout de 15s
    sachant de time augmente de 1 par 1/30s, à 15s ou 30s ou 1min, timer sera à 450 ou 900 ou 1800
    la fonction n'est appelée que lorsque time est à 450 ou 900 ou 1800
    on augmente la vélocité de la balle y et en x si elle est toujours de 1 en la multipliant par 1.1
...
def update():
    ...
    if life > 0:
        ...
        if 35 >= ball_y >= 33 or 29 <= ball_y+3 <= 32:
        ...
        #appel de la fonction si time est multiple de 15s
        if time % 450 == 0:
            ball_velocity_x, ball_velocity_y = timer(ball_velocity_x, ball_velocity_y)
        time = time+1
    ...
    if life > 0:
        if briques == [0, 0, 0, 0, 0, 0]:
            ...
            for i in range(len(briques)):
                if briques[i] != 0:
                    color = [0, 12, 5, 1]
                    pyxel.rect(briques_x, briques_y, 15, 4, color[briques[i]])
                briques_x = briques_x + 18

            #création de la balle de rayon 2
            pyxel.circ(int(ball_x), int(ball_y), 2, 7)
